      ...
      while (!file.eof () )
        {
          ...
          if (IsSetInitialPos (pr))
          ...
          else // NOW EVENTS TO BE SCHEDULED
            {
              ...
              if ( at < 0 )
                {
                  NS_LOG_WARN ("Time is less than zero: " << at);
                  continue;
                }
              ...
              if (IsSchedMobilityPos (pr))
                {
                  ...
                  // Log new position
                  NS_LOG_DEBUG ("Positions new at "<< at <<" after parse for node " << iNodeId << " " << nodeId << " position =" << last_pos[iNodeId].m_finalPosition);
                }
