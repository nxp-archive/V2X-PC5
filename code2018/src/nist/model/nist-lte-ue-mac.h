...
private:
  ...
  //added by adel
  //std::vector< std::vector<float> > m_signalpersubframelast100;
  std::list< std::vector<float> > m_signalpersubframe;
  double m_Threshold;
  double m_ThresholdExcludeResource;
  int m_selectionwindow;
  std::list < std::pair< std::pair<int,int>,std::pair<int,int> > > m_ReservedResources; //adde by adel //std::list< std::pair< std::pair<int,int>, int > >
  double m_CRTimeMin; //added by Adel
  std::list <int> m_usedresources; //added by adel
  ...
  uint32_t m_oldTxQueueSize; //added by adel: used to store old tx queue size
  int m_reselectioncounter;//added by adel
  uint16_t m_selectedsubframe;//added by adel
  uint16_t m_selectedsubchannel;//added by adel
  float m_probabilityKeepResource; //added by adel
  uint16_t m_reservenexttime; //added by adel
  bool m_DCC; //added by Adel
  bool m_firsttransmission; //added by Adel
  int m_oldresource; //added by Adel
 ...
 //added by Adel
 /**
  * Function to calculate average signal per subchannel.
  * Add average signal in a circular buffer to store latest 1000 values.
  */
  void DoCalculateReceivedSignal(Ptr<SpectrumValue> signal);
  /**
   * Function to get reserved resources by other vehicles
   */
  void DoGetAllReservedResources (std::list < std::pair< std::pair<int,int>,std::pair<int,int> > > ReservedResources);
  /**
   * Function to calculate CBR.
   * Takes into consideration signal per subchannel in the latest 100 subframes.
   * ETSI TS 136 214 V14.2.0
   ...
   * or PSCCH, in a pool (pre)configured such that PSCCH may be transmitted with its corresponding PSSCH in non-adjacent resource blocks, the portion of the resources of
   * the PSCCH pool whose S-RSSI measured by the UE exceed a (pre-)configured threshold sensed over subframes [n-100, n-1], assuming that the PSCCH pool is composed of
   * resources with a size of two consecutive PRB pairs in the frequency domain."
   */
  float DoCalculateCBR();
  /**
   * Function to calculate CR.
   * ETSI TS 136 214 V14.2.0
   * "Channel occupancy ratio (CR) evaluated at subframe n is defined as the total number of sub-channels used for its transmissions in subframes [n-a, n-1]
   * and granted in subframes [n, n+b] divided by the total number of configured sub-channels in the transmission pool over [n-a, n+b]."
   */
  float DoCalculateCR();
  /**
   * Function to select resources.
   * Sensing-based resource selection.
   */
  void DoSelectResource(uint16_t nbSubchannels);
  /**
   * Check DCC if we will transmit or not in application of Packet dropping
   */
  void DoDccCheck ();
