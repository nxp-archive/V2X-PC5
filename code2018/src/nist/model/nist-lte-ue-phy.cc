...
{
  NS_LOG_FUNCTION (this);
  NS_LOG_INFO("m_subChannelsForTransmission" << m_subChannelsForTransmission.size());
	...
	for (it = msgList.begin (); it != msgList.end (); it++)
	  {
		...
		if (msg->GetMessageType () == NistLteControlMessage::SCI)
		        { //it is here for sci
		                  ...
		                  else
		                    {
		                      ...
		                      //if (grantIt == poolIt->m_currentGrants.end())
		                        //{
		                          ...
		                          if (sci.m_reservenexttime == 1)
		                          {
		                        	  //The vehicle reserve the same respurce for next transmission
		                        	  SidelinkCommResourcePool::SubframeInfo now;
		                        	  now.frameNo = m_FrameNo;
		                        	  now.subframeNo = m_SubframeNo;
		                        	  SidelinkCommResourcePool::SubframeInfo reserveressource; //reserveressource is 100 subframes
		                        	  reserveressource.frameNo = 10; //reserved resource after 100 ms (for 10 Hz transmission) => To have general case later
		                        	  reserveressource.subframeNo = 0;
		                        	  //set the resource to be reserved
		                        	  reserveressource = reserveressource + now;

		                        	  if (reserveressource.subframeNo == 0)
		                        	  		{
		                        	  			reserveressource.frameNo +=1;
		                        	  			//reserveressourceInfo.subframeNo = 10;
		                        	  		}

		                        	  if (reserveressource.frameNo >1024)
		                        	  {
		                        		  reserveressource.frameNo = reserveressource.frameNo - 1024;
		                        	  }

		                        	  NS_LOG_INFO("now.frameNo= "<<now.frameNo<<" now.subframeNo= "<<now.subframeNo<<" reserveressource.frameNo= "<< reserveressource.frameNo<<" reserveressource.subframeNo= "<<reserveressource.subframeNo);
		                        	  NS_LOG_INFO(reserveressource.frameNo<<" "<<reserveressource.subframeNo);
		                        	  std::pair<int,int> frameinfo = std::make_pair(reserveressource.frameNo,reserveressource.subframeNo);
		                        	  std::pair<int,int> subchannels = std::make_pair(div(sci.m_rbStart,sci.m_SlGrantSize).quot, div(sci.m_rbLen,sci.m_SlGrantSize).quot);
		                        	  //add resource to all reserved resources
		                        	  m_allreservedresources.push_back( std::make_pair(frameinfo, subchannels) );
		                          }
		                          ...
		                          //reception of PSSCH
		                          NS_LOG_INFO (this << " Expecting PSSCH reception RB " << (uint16_t) sci.m_rbStart + 2 << " to " << (uint16_t) (sci.m_rbStart + sci.m_rbLen - 1));
		                          std::vector <int> rbMap;
		                          for (int i = sci.m_rbStart + 2; i < sci.m_rbStart + sci.m_rbLen; i++)
		                          {
		                               rbMap.push_back (i);
		                          }
		                          m_sidelinkSpectrumPhy->AddExpectedTb (sci.m_rnti, sci.m_groupDstId, 1, sci.m_tbSize, sci.m_mcs, rbMap, 4);
...
//             NS_LOG_DEBUG(this << " RNTI " << m_rnti << " RBG " << i << " DL-DCI allocated PRB " << (i*GetRbgSize()) + k);
          ...
          for (uint8_t i = 0; i < dci.m_tbsSize.size (); i++)
            {
              m_downlinkSpectrumPhy->AddExpectedTb (dci.m_rnti, dci.m_ndi.at (i), dci.m_tbsSize.at (i), dci.m_mcs.at (i), dlRb, i, dci.m_harqProcess, dci.m_rv.at (i), true / * DL * /); //true => true / * DL * /
            }
...
void
NistLteUePhy::GetAllSignals()
{
	Ptr<SpectrumValue> all = m_sidelinkSpectrumPhy->GetAllSignals();
	/*if (all)
	{
		NS_LOG_INFO ("all signals = "<<(*all));
	}*/

    m_uePhySapUser->CalculateReceivedSignal(all);
}

void
NistLteUePhy::GetAllReservedResources()
{
	m_uePhySapUser->GetAllReservedResources(m_allreservedresources);
}
...
NistLteUePhy::SubframeIndication (uint32_t frameNo, uint32_t subframeNo)
{
	  ...
	  std::list < std::pair< std::pair<int,int>,std::pair<int,int> > >::iterator rsrc= m_allreservedresources.begin();
      while ((rsrc != m_allreservedresources.end()) && ( (rsrc->first.first < (int)frameNo) || (rsrc->first.first == (int)frameNo && rsrc->first.second < (int)subframeNo) ))
      {
    		  m_allreservedresources.pop_front();
    		  rsrc= m_allreservedresources.begin();
      }
		 ...
		 // }
	      ...
	      if (rbMask.size() != 0 || (ctrlMsg.size () > 0 && (*ctrlMsg.begin())->GetMessageType () != NistLteControlMessage::SCI
	       ...
	       //uplink transmissions
    		   ...
    		   std::vector <int> slRb;
	    	   if ((ctrlMsg.size ()>0 && sciDiscfound) || pb)
	    	   {
	    		   std::list<Ptr<NistLteControlMessage> >::iterator msgIt = ctrlMsg.begin();
	    		   NS_LOG_INFO((*msgIt)->GetMessageType ());
		    	 // send PSCCH (ideal: fake null bandwidth signal)
	    		 ...
	    		 else if ((*msgIt)->GetMessageType () == NistLteControlMessage::SCI)
	    		 {
	    			 ...
	    			 while (grantIt != m_slTxPoolInfo.m_currentGrants.end ())
	    			 {
	    				 NS_LOG_INFO (this << " m_resPscch = "<< grantIt->second.m_grant.m_resPscch);
	    				 grantIt ++;
	    			 }
	    			 ...
	    			 if (grantIt == m_slTxPoolInfo.m_currentGrants.end ())
	    			 {
	    				 ...
	    				 NS_LOG_INFO (this << " m_resPscch = "<< (float) sci.m_resPscch << " m_rbStart = "<< (float) sci.m_rbStart << " m_rbLen = " << (float) sci.m_rbLen);

	    				 //grantInfo.m_pscchTx = m_slTxPoolInfo.m_pool->GetPscchTransmissionsV2X (0,sci.m_rbStart);
	    				 //grantInfo.m_psschTx = m_slTxPoolInfo.m_pool->GetPsschTransmissionsV2X (0,sci.m_rbStart,sci.m_rbLen);

	    				 //for (std::list<SidelinkCommResourcePool::SidelinkTransmissionInfo>::iterator txIt = grantInfo.m_psschTx.begin (); txIt != grantInfo.m_psschTx.end (); txIt++)
	    				 //{
	    				 ...
	    				 //std::vector <int> slRb;
	    				 NS_LOG_LOGIC (this << " pscch rbstart = " << (float) txIt ->rbStart << " rblen = " << (float) txIt->nbRb);
	    				 ...
	    				 for (int i = sci.m_rbStart; i < sci.m_rbStart+2; i++)
	    				 {
	    					 NS_LOG_LOGIC (this << " Transmitting PSCCH on RB " << i);
	    					 slRb.push_back (i);
	    				 }
	               ...
	               if (m_enableUplinkPowerControl)
	                   {
	                       ...
	                       NS_LOG_LOGIC (this << " transmission power = " << m_txPower);
	                   }
