...
namespace ns3 {
  ...
  void
  SidelinkCommResourcePool::SetPoolV2X (NistLteRrcSap::SlPreconfigCommPool pool)
  {
    //this configuration is for V2X
    m_type = SidelinkCommResourcePool::V2X;
    m_preconfigured = false;
    //parse information
    m_scCpLen = pool.scCpLen;
    m_scPeriod = pool.scPeriod;
  ...
  SidelinkCommResourcePool::GetNextScPeriod (uint32_t frameNo, uint32_t subframeNo)
  {
    ...
    //std::cout << subframe << ", " << m_scTfResourceConfig.offsetIndicator.offset << ", " << (int32_t) (subframe - m_scTfResourceConfig.offsetIndicator.offset) << ", " << (subframe - m_scTfResourceConfig.offsetIndicator.offset) / period << std::endl;
    int32_t currentPeriod = std::floor ((subframe - m_scTfResourceConfig.offsetIndicator.offset) / (double) period);
    //std::cout<<" offset: " << (float) m_scTfResourceConfig.offsetIndicator.offset <<" frame no: " << (float) frameNo << " subframe no: " << (float) subframeNo << " subframe: " << (float) subframe<<" currentPeriod: " << (float) currentPeriod;
  ...
  SidelinkCommResourcePool::GetPscchTransmissions (uint32_t n)
  {
    ...
    //std::cout << " n= "<<(float) n <<" m_lpscch "<<(float) m_lpscch <<" subframe= " << (float) subframe;
    //std::cout << " first subframe "<< subframe;
    ...
    //std::cout << " second subframe "<< subframe;
  ...
  std::list<SidelinkCommResourcePool::SidelinkTransmissionInfo>
  SidelinkCommResourcePool::GetPscchTransmissionsV2X (uint32_t time, uint32_t subch)
  {
    //NS_ASSERT_MSG (time < m_nPscchResources, "Requesting resource " << time << " but max is " << m_nPscchResources);

    std::list<SidelinkCommResourcePool::SidelinkTransmissionInfo> trans;
    SidelinkCommResourcePool::SidelinkTransmissionInfo opportunity;

    opportunity.subframe.frameNo = time / 10;
    opportunity.subframe.subframeNo = time % 10;
    opportunity.rbStart = subch;
    opportunity.nbRb = 2;

    trans.push_back (opportunity);
  ...
  SidelinkCommResourcePool::GetPsschTransmissions (SidelinkCommResourcePool::SubframeInfo periodStart, uint8_t itrp, uint8_t rbStart, uint8_t rbLen)
  {
    ...
    std::vector<uint32_t> psschsubframes;
    std::cout << "m_lpssch= " << (float) m_lpssch;
    for (uint32_t i=0; i < m_lpssch; i++)
      {
        //compute b_i = b'_(i mod N_TRP)
        //std::cout << "i=" << i << ", i%ntrp=" << (i % ntrp) << ", b'=" << bitmap[i % ntrp] << std::endl;
  ...
  std::list<SidelinkCommResourcePool::SidelinkTransmissionInfo>
    SidelinkCommResourcePool::GetPsschTransmissionsV2X (uint32_t time, uint32_t subch, uint32_t subchsize)
  {
	  std::list<SidelinkCommResourcePool::SidelinkTransmissionInfo> trans;
	  SidelinkCommResourcePool::SidelinkTransmissionInfo opportunity;
	  ...
	  opportunity.subframe.frameNo = time / 10;
	  opportunity.subframe.subframeNo = time % 10;
	  opportunity.rbStart = subch + 2; //first 2 are dedicated for pscch
	  opportunity.nbRb = subchsize-2;
	  //std::cout<<"number of RBs for PSSCH= "<< (float)(subchsize-2)<<"\n";
	  trans.push_back (opportunity);
  ...
  /*SidelinkCommResourcePool::SidelinkTransmissionInfo
  SidelinkCommResourcePool::TranslatePscchV2X (SidelinkCommResourcePool::SidelinkTransmissionInfo info)
   {
	    SidelinkCommResourcePool::SidelinkTransmissionInfo translatedInfo;
    ...
    for (int i = m_scTfResourceConfig.prbStart ; i <= m_scTfResourceConfig.prbEnd ; i++)
      {
        if (i < m_scTfResourceConfig.prbStart + m_scTfResourceConfig.prbNum
            || i > m_scTfResourceConfig.prbEnd - m_scTfResourceConfig.prbNum)
          {
        	...
        	//m_scTfResourceConfig.prbNum=22 m_scTfResourceConfig.prbStart=0 m_scTfResourceConfig.prbEnd=49 i=0 21 28 48
        	//std::cout <<" i= "<<(float) i<<" m_scTfResourceConfig.prbStart= " <<(float) m_scTfResourceConfig.prbStart << " m_scTfResourceConfig.prbNum " <<(float) m_scTfResourceConfig.prbNum<<" m_scTfResourceConfig.prbEnd= " <<(float) m_scTfResourceConfig.prbEnd << " m_scTfResourceConfig.prbNum= "<<(float) m_scTfResourceConfig.prbNum;
            m_rbpscchVector.push_back ((uint32_t)i);
          }
      }
    m_rbpscch = m_rbpscchVector.size();
    m_nPscchResources = std::floor (m_lpscch * m_rbpscch / 2);
    //m_lpscch=8 m_rbpscch=44
    //std::cout << "L_pscch=" << m_lpscch << ", m_rb=" << (float) m_rbpscch << ", m_nPscchResources=" << (float) m_nPscchResources << std::endl;
  }
  ...
  void
  SidelinkTxCommResourcePool::SetPoolV2X (NistLteRrcSap::SlPreconfigCommPool pool)
  {
    SidelinkCommResourcePool::SetPoolV2X (pool);
    m_index = 0;
    m_mcs = 0;
    m_scTxParameters = pool.scTxParameters;
    m_dataTxParameters = pool.dataTxParameters;
