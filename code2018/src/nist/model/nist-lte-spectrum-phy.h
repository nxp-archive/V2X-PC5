...
private:
  ...
  void EndRxDlCtrl ();
  void EndRxUlSrs ();
  void EndRxSlData (Ptr<NistLteSpectrumSignalParametersSlFrame> params);
  
  void SetTxModeGain (uint8_t txMode, double gain);
  
  double GetMeanSinr (const SpectrumValue& sinr, const std::vector<int>& map);
  double GetMeanSinrfromSignal (const SpectrumValue& signal, const std::vector<int>& map);
  ...
  //Added by Adel
  std::vector < Ptr<NistLteSpectrumSignalParametersSlFrame> > m_allsendIDs;
};
