  ...
  if ( m_slRxFirstWrite == true )
    {
      ...
      outFile << "% time\tcellId\tIMSI\tRNTI\tlayer\tmcs\tsize\trv\tndi\tcorrect\tdistance\treason";
      outFile << std::endl;
    }
  else
    {
...
//   outFile << Simulator::Now ().GetNanoSeconds () / (double) 1e9 << "\t";
  ...
  outFile << (uint32_t) params.m_ndi << "\t";
  outFile << (uint32_t) params.m_correctness << "\t";
  outFile << (uint32_t) params.m_distance <<"\t";
  outFile << params.m_reason << std::endl;
  outFile.close ();
}
  ...
  if ( m_slPscchRxFirstWrite == true )
    {
      ...
      outFile << "% time\tcellId\tIMSI\tRNTI\tlayer\tcorrect\tdistance\treason";
