...
void SlStartDiscovery (Ptr<NistLteHelper> helper, Ptr<NetDevice> ue, std::list<uint32_t> apps, bool rxtx)
{
  helper->StartDiscovery (ue, apps, rxtx);
}

void SlStopDiscovery (Ptr<NistLteHelper> helper, Ptr<NetDevice> ue, std::list<uint32_t> apps, bool rxtx)
{
...
{  
  ...
  *st->GetStream () << "id\tx\ty" << std::endl;
  ...
  preconfiguration.preconfigDisc.pools[0].discPeriod.period = NistLteRrcSap::SlPeriodDisc::rf32; //sidelink period for discovery application
