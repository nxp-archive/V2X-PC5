...
FriisPropagationLossModel::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::FriisPropagationLossModel")
    ...
    .AddAttribute ("Frequency", 
                   ...
                   DoubleValue (5.950e9), //old=5.150e9
...
FriisPropagationLossModel::DoCalcRxPower (double txPowerDbm,
                                          ...
                                          Ptr<MobilityModel> b) const
{
	NS_LOG_FUNCTION(txPowerDbm<<a<<b);
  ...
  NS_LOG_INFO(" TX power = "<<txPowerDbm<<" dbm  m_lambda= "<< m_lambda <<" m_systemLoss= "<< m_systemLoss <<" m_minLoss= "<<m_minLoss);
  ...
  //std::cout<< (float)distance<<" "<< (float)lossDb<<"\n";
...
NS_OBJECT_ENSURE_REGISTERED (NLOSPropagationLossModel);


TypeId
NLOSPropagationLossModel::GetTypeId (void)
{
...
NLOSPropagationLossModel::NLOSPropagationLossModel()
{

}

double
NLOSPropagationLossModel::DoCalcRxPower (double txPowerDbm,
        									Ptr<MobilityModel> a,
											Ptr<MobilityModel> b) const
	{
	NS_LOG_FUNCTION(txPowerDbm<<a<<b);
	...
	//NLOS
	double lossdb = 11.86 + 27.67 * log10 (distance*distance/2) ;

	NS_LOG_INFO("distance = "<< distance << " lossdb= "<<lossdb);

	return txPowerDbm - lossdb;
	}

int64_t
NLOSPropagationLossModel::DoAssignStreams (int64_t stream)
{
  return 0;
}
...
NS_OBJECT_ENSURE_REGISTERED (LOSNLOSPropagationLossModel);

TypeId
LOSNLOSPropagationLossModel::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::LOSNLOSPropagationLossModel")
    .SetParent<PropagationLossModel> ()
    .AddConstructor<LOSNLOSPropagationLossModel> ()
	.AddAttribute ("factor",
	               "factor to adjust LOS/NLOS",
	               DoubleValue (0.0),
	               MakeDoubleAccessor (&LOSNLOSPropagationLossModel::m_factor),
	               MakeDoubleChecker<double> ())
	.AddAttribute ("frequency",
				   "frequency value: by default 5.9Ghz ",
				   DoubleValue (5950),
				   MakeDoubleAccessor (&LOSNLOSPropagationLossModel::m_frequency),
				   MakeDoubleChecker<double> ())
	.AddAttribute ("DistanceBetween2ParallelRoad",
				   "DistanceBetween2ParallelRoad in the Manhatten grid:default 200m",
				   DoubleValue (200),
				   MakeDoubleAccessor (&LOSNLOSPropagationLossModel::m_distance),
				   MakeDoubleChecker<double> ())
  ;
  return tid;
}

LOSNLOSPropagationLossModel::LOSNLOSPropagationLossModel()
{
}

double
LOSNLOSPropagationLossModel::DoCalcRxPower (double txPowerDbm,
        									Ptr<MobilityModel> a,
											Ptr<MobilityModel> b) const
{
	...
	/*float adjust = exp(-distance*m_factor);
	NS_LOG_INFO("distance= "<< distance <<" m_factor= "<<m_factor<<" adjust= "<< adjust);

	double lossdbLOS = 20 * log10 (4*M_PI*distance*m_frequency*100000/300000000);
	double lossdbNLOS = 11.86 + 27.67 * log10 (distance*distance/2);
	double lossdb = adjust * lossdbLOS + (1-adjust) * lossdbNLOS;
	NS_LOG_INFO("lossdbLOS= "<<lossdbLOS<<" lossdbNLOS= "<< lossdbNLOS <<" lossdb= "<<lossdb);*/
	...
	if (m_factor == 0.0)
	{
		//LOS
		NS_LOG_INFO("LOS");
		//double pbdist = 4 * 0.5 * 0.5 * m_frequency * 10000000 / (300000000.0);
		lossdb = PathLossLOS(distance,m_frequency);
		NS_LOG_INFO("lossdb= "<<lossdb);
		//std::cout<<(float)distance<<" "<<(float)lossdb<<" "<<(float)txPowerDbm<< " "<<(float)(txPowerDbm-lossdb) <<"\n";
	} else
	{
		NS_LOG_INFO("NLOS");
		NS_LOG_INFO("position of vehicle 1: "<<a->GetPosition()<<" position of vehicle 2: "<<b->GetPosition());
		//MANHATTEN GRID: NLOS
		if ( (std::abs(a->GetPosition().x - b->GetPosition().x) < 20) || (std::abs(a->GetPosition().y - b->GetPosition().y) < 20) )
			{
				NS_LOG_INFO("2 vehicles are in the same street => LOS");
				//2 vehicles are in the same street => LOS
				lossdb = PathLossLOS(distance,m_frequency);
				NS_LOG_INFO("lossdb= "<<lossdb);

			} else if  ( (std::abs(a->GetPosition().x - b->GetPosition().x) > (m_distance- 20)) ||  (std::abs(a->GetPosition().y - b->GetPosition().y) > (m_distance- 20)) )
			{
				//2 vehicles are in parallel streets => lossdb = infinit
				NS_LOG_INFO("2 vehicles are in parallel streets => lossdb = infinit");
				//lossdb = txPowerDbm;
				lossdb = 1000;
				NS_LOG_INFO("lossdb= "<<lossdb);

			} else {
				// 2 vehicles are in cross roads => NLOS
				NS_LOG_INFO(" 2 vehicles are in cross roads => NLOS");

				double d1 = std::abs(a->GetPosition().x - b->GetPosition().x );
				double d2 = std::abs(a->GetPosition().y - b->GetPosition().y );

				double lossdb1  = PathLoss (d1,d2,m_frequency);
				double lossdb2 = PathLoss (d2,d1,m_frequency);
				lossdb = std::min(lossdb1,lossdb2);
				NS_LOG_INFO("lossdb= "<<lossdb);
...
double
LOSNLOSPropagationLossModel::PathLossLOS (double distance, double frequency)
{
	double lossdbLOS;
	double pbdist = 4 * 0.5 * 0.5 * frequency * 10000000 / (300000000.0);

	if (distance < pbdist)
	{
		lossdbLOS = 22.7 * log10(distance)+ 41.0 + 20 * log10(frequency/(5.0*1000.0));

	} else {
		lossdbLOS = 40.0 * log10(distance) + 9.45 - 2 * 17.3 * log10(0.5) + 2.7 * log10(frequency/(5.0*1000.0));
	}
		return (lossdbLOS);
	...
	if (distance < 3*m_lambda)
	  ...
	  double numerator = m_lambda * m_lambda;
	  double denominator = 16 * M_PI * M_PI * distance * distance * 100;
	  double lossDb = -10 * log10 (numerator / denominator);
	  NS_LOG_DEBUG ("distance=" << distance<< "m, loss=" << lossDb <<"dB");
	  //std::cout<< (float)distance<<" "<< (float)lossDb<<"\n";
...
double
LOSNLOSPropagationLossModel::PathLoss (double d1, double d2, double frequency)
{
	double lossdbNLOS;
	double Nj = std::max(2.8 - 0.0024 * d1 , 1.84);

	lossdbNLOS = PathLossLOS(d1,frequency) + 20 - 12.5 * Nj + 10 * Nj * log10(d2) + 3 * log10(frequency/(5.0*1000.0)) ;
...
int64_t
LOSNLOSPropagationLossModel::DoAssignStreams (int64_t stream)
{
  return 0;
}
