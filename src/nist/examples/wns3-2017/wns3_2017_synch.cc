...
void
NotifyChangeOfSyncRef (Ptr<OutputStreamWrapper> stream, NistLteUeRrc::SlChangeOfSyncRefStatParameters param)
{
...
void
NotifyChangeOfSyncRefAndReportNumberOfTimings (Ptr<OutputStreamWrapper> stream, NistLteUeRrc::SlChangeOfSyncRefStatParameters param)
{
...
NS_LOG_COMPONENT_DEFINE ("NistExampleD2DTestMergeCommAndSync");
int
main (int argc, char *argv[])
{
  ...
  Config::SetDefault ("ns3::NistLteEnbRrc::DefaultTransmissionMode", UintegerValue (1));// TxDiversity(1); SpatialMultiPlex(2)
  ...
  // Set error models
  Config::SetDefault ("ns3::NistLteSpectrumPhy::SlDataBLERModelEnabled", BooleanValue (false));
  Config::SetDefault ("ns3::NistLteSpectrumPhy::NistErrorModelEnabled", BooleanValue (true));
  Config::SetDefault ("ns3::NistLteSpectrumPhy::FadingModel", StringValue ("AWGN"));
  ...
  Config::SetDefault ("ns3::NistLteEnbRrc::SrsPeriodicity", UintegerValue (160));
  ...
  Config::SetDefault ("ns3::NistLteUePhy::UeSlssInterScanningPeriodMax", TimeValue (MilliSeconds(interScanTimeMax)));
  Config::SetDefault ("ns3::NistLteUePhy::UeSlssInterScanningPeriodMin", TimeValue (MilliSeconds(interScanTimeMin)));
  Config::SetDefault ("ns3::NistLteUePhy::UeSlssScanningPeriod", TimeValue (MilliSeconds(scanTime)));
  Config::SetDefault ("ns3::NistLteUePhy::UeSlssMeasurementPeriod", TimeValue (MilliSeconds(measTime)));
  Config::SetDefault ("ns3::NistLteUePhy::UeSlssEvaluationPeriod", TimeValue (MilliSeconds(evalTime)));
  Config::SetDefault ("ns3::NistLteUePhy::UeRandomInitialSubframeIndication", BooleanValue(unsyncSl));
  if (slSyncActive)
    {
      Config::SetDefault ("ns3::NistLteUeRrc::UeSlssTransmissionEnabled", BooleanValue(true));
    }
  Config::SetDefault ("ns3::NistLteUeRrc::MinSrsrp", DoubleValue(-125));
  Config::SetDefault ("ns3::NistLteUePhy::MinSrsrp", DoubleValue(-125));
  Config::SetDefault ("ns3::NistLteUePhy::NSamplesSrsrpMeas", UintegerValue(4));
  /* END Synchronization*/
  ...
  Config::SetDefault ("ns3::NistLteSpectrumPhy::DropRbOnCollisionEnabled", BooleanValue (CtrlDropOnCollisionEnabled));//If true, PSCCH drop msg when collision occurs, regardless SINR
      ...
      for (uint32_t i = 0; i < ueResponders.GetN (); ++i)
        {
          ...
          m_outFile << " UE\t" << ueNetDevice->GetObject<NistLteUeNetDevice> ()->GetImsi () << "\t" << ueResponders.Get (i)->GetObject<MobilityModel> ()->GetPosition ().x << "\t" << ueResponders.Get (i)->GetObject<MobilityModel> ()->GetPosition ().y << std::endl;
        }
      m_outFile.close ();
    }
  ////////////////////////////////////////////////////////////////////
  ...
  double ulEarfcn = enbDevs.Get (0)->GetObject<NistLteEnbNetDevice> ()->GetDlEarfcn ();
  double ulBandwidth = enbDevs.Get (0)->GetObject<NistLteEnbNetDevice> ()->GetDlBandwidth ();
  std::vector < NetDeviceContainer > createdgroups;

  //createdgroups = proseHelper->AssociateForBroadcastWithTxEnabledToReceive (ueTxPower, ulEarfcn, ulBandwidth, ueRespondersDevs, -1000, nbgroups, NistLteProseHelper::SRSRP_EVAL);
  createdgroups = proseHelper->AssociateForBroadcast (ueTxPower, ulEarfcn, ulBandwidth, ueRespondersDevs, -112, nbgroups, NistLteProseHelper::SRSRP_STD);
  ...
  *stream->GetStream () << "time\ttx/rx\tNID\tIMSI\tUEtype\tsize\tIP[src]\tIP[dst]" << std::endl;
  ...
  for (gIt = createdgroups.begin() ; gIt != createdgroups.end() ; gIt++)
    {
      ...
      Ptr<NistSlTft> tft = Create<NistSlTft> (NistSlTft::TRANSMIT, groupRespondersIpv4Address, groupL2Address);
      proseHelper->ActivateSidelinkBearer (Seconds(1.0), txUe, tft); 
      tft = Create<NistSlTft> (NistSlTft::RECEIVE, groupRespondersIpv4Address, groupL2Address);
      ...
      if (onoff) 
        ...
        {
          // UDP application
          NistUdpEchoClientHelper echoClientHelper (groupRespondersIpv4Address, gecho_server_port);
      ...
      for (uint16_t ac = 0; ac < clientRespondersApps.GetN (); ac++)
        {
          oss << "t\t" << (*gIt).Get(ac)->GetNode()->GetId () << "\t" << (*gIt).Get(ac)->GetObject<NistLteUeNetDevice> ()->GetImsi () << "\tresp";   
  ...
  NistPacketSinkHelper clientPacketSinkHelper ("ns3::UdpSocketFactory", InetSocketAddress (Ipv4Address::GetAny (), echo_port));
  ...
  //control 
  preconfiguration.preconfigComm.pools[0].scCpLen.cplen = NistLteRrcSap::SlCpLen::NORMAL;
  //preconfiguration.preconfigComm.pools[0].scPeriod.period = NistLteRrcSap::SlPeriodComm::sf40;
  preconfiguration.preconfigComm.pools[0].scPeriod.period = NistLteRrcSap::PeriodAsEnum(sl_period).period;
  ...
  //data
  preconfiguration.preconfigComm.pools[0].dataCpLen.cplen = NistLteRrcSap::SlCpLen::NORMAL;
  preconfiguration.preconfigComm.pools[0].dataHoppingConfig.hoppingParameters = 0;
  preconfiguration.preconfigComm.pools[0].dataHoppingConfig.numSubbands = NistLteRrcSap::SlHoppingConfigComm::ns4;
  ...
  preconfiguration.preconfigComm.pools[0].scTxParameters.alpha = NistLteRrcSap::SlTxParameters::al09;
  preconfiguration.preconfigComm.pools[0].scTxParameters.p0 = -40;
  preconfiguration.preconfigComm.pools[0].dataTxParameters.alpha = NistLteRrcSap::SlTxParameters::al09;
  ...
  *streamFirstScan->GetStream () << "Time\tIMSI" << std::endl;
      ...
      for (uint32_t i = 0; i < (*groupIt).GetN (); ++i)
        {
          ...
          (*groupIt).Get (i)->GetObject<NistLteUeNetDevice> ()->GetRrc ()->SetSlssid(slssid);

          if ((*groupIt).Get (i)->GetObject<NistLteUeNetDevice> ()->GetPhy ()->GetFirstScanningTime() == MilliSeconds (0)){
              uint32_t t = 0;
              if(slSyncActive){
                  t = rndStartScanning->GetInteger();
                  (*groupIt).Get (i)->GetObject<NistLteUeNetDevice> ()->GetPhy ()->SetFirstScanningTime(MilliSeconds(t));
              }
              *streamFirstScan->GetStream () << t <<"\t" << (*groupIt).Get (i)->GetObject<NistLteUeNetDevice> ()->GetRrc ()->GetImsi() << std::endl;
          }
        }
    }
  ...
  for (groupIt = createdgroups.begin() ; groupIt != createdgroups.end() ; groupIt++)
    {
      uint64_t grupHeadImsi = (*groupIt).Get (0)->GetObject<NistLteUeNetDevice> ()->GetRrc ()->GetImsi();
      uint64_t grupHeadSlssid =  (*groupIt).Get (0)->GetObject<NistLteUeNetDevice> ()->GetRrc ()->GetSlssid();

      for (uint32_t i = 0; i < (*groupIt).GetN (); ++i)
        {
          uint64_t ueImsi =  (*groupIt).Get (i)->GetObject<NistLteUeNetDevice> ()->GetRrc ()->GetImsi();
          uint64_t ueSlssid =  (*groupIt).Get (i)->GetObject<NistLteUeNetDevice> ()->GetRrc ()->GetSlssid();
      ...
      NistLteUeRrc::SlChangeOfSyncRefStatParameters param;
  ...
  for (uint32_t i = 0; i < ueRespondersDevs.GetN (); ++i)
    {
      ...
      Ptr<NistLteUeRrc> ueRrc =  ueRespondersDevs.Get (i)->GetObject<NistLteUeNetDevice> ()->GetRrc ();
  ...
  for (groupIt = createdgroups.begin() ; groupIt != createdgroups.end() ; groupIt++)
    {
      uint64_t grupHeadImsi = (*groupIt).Get (0)->GetObject<NistLteUeNetDevice> ()->GetRrc ()->GetImsi();
      uint64_t groupHeadSlssid =   (*groupIt).Get (0)->GetObject<NistLteUeNetDevice> ()->GetRrc ()->GetSlssid();
