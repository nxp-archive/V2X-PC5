...
NistLteSlInterference::SetNoisePowerSpectralDensity (Ptr<const SpectrumValue> noisePsd)
{
  ...
  m_sensedSignal = Create<SpectrumValue> (noisePsd->GetSpectrumModel ());
  m_oldsensedSignal = Create<SpectrumValue> (noisePsd->GetSpectrumModel ());
...
Ptr<SpectrumValue>
NistLteSlInterference::GetAllSignals()
{
	//NS_LOG_FUNCTION (this<<(*m_allSignals));
	//return m_allSignals;

	//m_sentsensedSignal = m_sensedSignal->Copy();
	if (m_sensedSignal)
	{
		NS_LOG_FUNCTION (this<<(*m_sensedSignal));
		(*m_sensedSignal) -= (*m_oldsensedSignal);
		(*m_oldsensedSignal)=(*m_sensedSignal);

		(*m_sensedSignal) += (*m_noise);
	}
	return m_sensedSignal;
}
