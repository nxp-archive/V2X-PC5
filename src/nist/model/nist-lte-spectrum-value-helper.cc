...
static const struct EutraChannelNumbers
{
  ...
  { 47, 5855, 54540, 54540, 55239, 5855, 54540, 54540, 55239} //added for lte v2x
};
...
NistLteSpectrumValueHelper::GetSpectrumModel (uint16_t earfcn, uint8_t txBandwidthConfiguration)
{
  ...
  if (it != g_lteSpectrumModelMap.end ())
  ...
  else
    {
      ...
      NS_LOG_INFO("carrier frequency: fc= "<< fc); //for sidelink 5.9e+9
...
NistLteSpectrumValueHelper::CreateNoisePowerSpectralDensity (double noiseFigureDb, Ptr<SpectrumModel> spectrumModel)
{
  ...
  //add noise bandwidth @99% power
  //added by Adel for v2x to have the right rx noise floor
  double noiseBandwith = 1;
  //noiseBandwith =10*1000000;
  double noisePowerSpectralDensity =  kT_W_Hz * noiseFigureLinear * noiseBandwith;
  double noisePowerSpectralDensitydD = 10 * std::log10 (noisePowerSpectralDensity);
  NS_LOG_INFO("kT_dBm_Hz= "<<kT_dBm_Hz<<" kT_W_Hz= "<<kT_W_Hz<<" noiseFigureDb= "<<noiseFigureDb<<" noiseFigureLinear "<<noiseFigureLinear<<"noisePowerSpectralDensity"<<noisePowerSpectralDensity<<" noisePowerSpectralDensitydD= "<<noisePowerSpectralDensitydD);

  Ptr<SpectrumValue> noisePsd = Create <SpectrumValue> (spectrumModel);
  (*noisePsd) = noisePowerSpectralDensity /* /(50*180000) */; //adding 50*180000
  NS_LOG_INFO("noisePsd= "<< (*noisePsd));
  return noisePsd;
}
