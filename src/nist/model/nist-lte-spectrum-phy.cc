...
NistLteSpectrumPhy::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::NistLteSpectrumPhy")
    ...
    .AddAttribute ("FadingModel",
                   "Fading model",
                   EnumValue (NistLtePhyErrorModel::EVA), //old: AWGN
                   MakeEnumAccessor (&NistLteSpectrumPhy::m_fadingModel),
                   MakeEnumChecker (NistLtePhyErrorModel::EVA, "EVA")) //old: AWGN, "AWGN"
...
NistLteSpectrumPhy::StartRx (Ptr<SpectrumSignalParameters> spectrumRxParams)
{
  ...
  if (lteDataRxParams != 0)
    ...
    {
	  NS_LOG_INFO(m_halfDuplexPhy->GetState ());
      ...
      {
    	 NS_LOG_INFO("half-duplex");
        StartRxSlData (lteSlRxParams);
      }
      else
      {
      	 NS_LOG_INFO("no half-duplex");
      }
    } 
  else
    {
...
{
  NS_LOG_FUNCTION (this);
  NS_LOG_INFO ("params= "<<params->nodeId);
  NS_LOG_LOGIC (this << " ID:" << GetDevice()->GetNode()->GetId() << " state: " << m_state);
                        ...
                        if (m_rxPacketInfo.empty ())
                          {
                            ...
                            m_endRxDataEvent = Simulator::Schedule (params->duration, &NistLteSpectrumPhy::EndRxSlData, this, params);
                          }
                        else
                          {
                            NS_ASSERT (m_state == RX_DATA);
            ...
            if (params->slssId == m_slssId && (params->groupId == 0 || m_l1GroupIds.find (params->groupId) != m_l1GroupIds.end()))
              {
                ...
                ChangeState (RX_DATA); //IDLE -> RX_DATA
                m_interferenceSl->StartRx (params->psd);
...
NistLteSpectrumPhy::StartRxCtrl (Ptr<SpectrumSignalParameters> params)
{
  ...
  switch (m_state)
  {
    case TX:
      ...
      break;
    case RX_DATA:
      NS_FATAL_ERROR ("cannot RX data while receving control");
      break;
    case IDLE:
    case RX_CTRL:
      // the behavior is similar when
...
void
NistLteSpectrumPhy::EndRxSlData (Ptr<NistLteSpectrumSignalParametersSlFrame> paramss)
{
  NS_LOG_FUNCTION (this);
  for (int i=0; i < (int)m_allsendIDs.size();i++)
  {
	  NS_LOG_INFO(m_allsendIDs[i]->nodeId);
  }

  NS_LOG_INFO ("paramss "<< paramss->nodeId);
  NS_LOG_LOGIC (this << " ID:" << GetDevice()->GetNode()->GetId() << " state: " << m_state);
  NS_ASSERT (m_state == RX_DATA);
  ...
  // Currently the MIB-SL is treated as a control message. Thus, the following logic applies also to the MIB-SL
    ...
    std::multiset<NistSlCtrlPacketInfo_t> sortedControlMessagesSingalSignal;
    std::set<int> rbDecodedBitmap;
    //rbDecodedBitmap.clear ();

    std::string reason_PSCCH;
    std::string reason_PSSCH;

    //Ptr<SpectrumValue> a = GetAllSignals();
    //NS_LOG_INFO ("all signals = "<<(*all));
        ...
        if (m_rxPacketInfo[i].m_rxControlMessage && m_rxPacketInfo[i].m_rxControlMessage->GetMessageType () != NistLteControlMessage::SL_DISC_MSG) //if control packet
          {
            ...
            pInfo.sinr = meanSinr;
            pInfo.index = i;
            NS_LOG_INFO("sortedControlMessages: index= "<< i <<" meansinr= " << meanSinr);
            sortedControlMessages.insert (pInfo);
            //ctrl_msg_count++;

            double meanSinrSingalSignal = GetMeanSinrfromSignal (m_slSignalPerceived[i], m_rxPacketInfo[i].rbBitmap);
            NistSlCtrlPacketInfo_t pInfoSingalSignal;
            pInfoSingalSignal.sinr = meanSinrSingalSignal;
            pInfoSingalSignal.index = i;
            sortedControlMessagesSingalSignal.insert (pInfoSingalSignal);
    ...
    for (std::multiset<NistSlCtrlPacketInfo_t>::iterator it =sortedControlMessages.begin(); it != sortedControlMessages.end() ; it++ )
      {
        int i = (*it).index;
        NS_LOG_INFO(" index= "<< (*it).index<< " sinr = " <<(*it).sinr );
            ...
            for (std::vector<int>::iterator rbIt =  m_rxPacketInfo[i].rbBitmap.begin ();  rbIt != m_rxPacketInfo[i].rbBitmap.end() ; rbIt++)
              {
                if (rbDecodedBitmap.find (*rbIt) != rbDecodedBitmap.end())
                  {
              	  NS_LOG_INFO("getting into if statement");
                    rbDecoded = true;
                    ctrlError = true;
                    NS_LOG_DEBUG (this << " RB " << *rbIt << " has already been decoded ");
                    for (std::multiset<NistSlCtrlPacketInfo_t>::iterator itSingalSignal =sortedControlMessagesSingalSignal.begin(); itSingalSignal != sortedControlMessagesSingalSignal.end() ; itSingalSignal++ )
                    {
                    	if ( (*it).index == (*itSingalSignal).index)
                    	{
                    		double sinrSingalSignal =  (*itSingalSignal).sinr;
                            double errorRatebis = NistLtePhyErrorModel::GetPscchBler (m_fadingModel,NistLtePhyErrorModel::SISO, sinrSingalSignal).tbler;
                            bool ctrlErrorbis = m_random->GetValue () > errorRatebis ? false : true;
                    		NS_LOG_INFO("sinrSingalSignal= "<<sinrSingalSignal<<" errorRatebis= "<< errorRatebis <<" ctrlErrorbis= "<< ctrlErrorbis);
                            if (ctrlErrorbis == 0)
                            {
                            	reason_PSCCH = "collision";
                            } else
                            {
                            	reason_PSCCH = "propagation path loss";
                            }
                    	}
                    }
                    //reason_PSCCH = "collision + propagation path loss";
                    break;
                  }
              }
                ...
                //if (!m_dropRbOnCollisionEnabled)//uncomment IF_CONDITION if PSCCH BLER is not considered in this mode!
                  //{
                    ...
                    if ( m_rxPacketInfo[i].m_rxControlMessage->GetMessageType() == NistLteControlMessage::SCI)
                      {
                        errorRate = NistLtePhyErrorModel::GetPscchBler (m_fadingModel,NistLtePhyErrorModel::SISO, GetMeanSinr (m_slSinrPerceived[i] /*(m_slSinrPerceived[i]*4) Average gain for SIMO based on [CatreuxMIMO] */, m_rxPacketInfo[i].rbBitmap)).tbler;
                        ctrlError = m_random->GetValue () > errorRate ? false : true;
                        //std::cout<<"PSCCH "<<(float) GetMeanSinr (m_slSinrPerceived[i], m_rxPacketInfo[i].rbBitmap)<<" "<<(float)m_mobility-> GetDistanceFrom (paramss->txPhy->GetMobility())<<" "<< (float)errorRate<<" "<<(float)ctrlError<<"\n";
                        //std::cout<<"PSCCH "<< (float)m_mobility-> GetDistanceFrom (paramss->txPhy->GetMobility())<<" "<< (float)errorRate<<" "<<(float)ctrlError<<"\n";
                        NS_LOG_INFO("SINR= "<< GetMeanSinr (m_slSinrPerceived[i], m_rxPacketInfo[i].rbBitmap));
                      ...
                      {
                        errorRate = NistLtePhyErrorModel::GetPsbchBler (m_fadingModel,NistLtePhyErrorModel::SISO, GetMeanSinr (m_slSinrPerceived[i] /*m_slSinrPerceived[i]*4: Average gain for SIMO based on [CatreuxMIMO] */, m_rxPacketInfo[i].rbBitmap)).tbler;
                        ctrlError = m_random->GetValue () > errorRate ? false : true;
        ...
        if(m_rxPacketInfo[i].m_rxControlMessage->GetMessageType () == NistLteControlMessage::SCI)
          {
            ...
            for (int i=0; i < (int)m_allsendIDs.size();i++)
             {
            	if ( m_allsendIDs[i]->nodeId == sci.m_rnti)
            	{
            		pscch_dist = m_mobility-> GetDistanceFrom (m_allsendIDs[i]->txPhy->GetMobility());
            	}
           	  //NS_LOG_INFO(m_allsendIDs[i]->nodeId);
             }
            params.m_distance = pscch_dist;
            params.m_reason = reason_PSCCH;
          ...
          if ((m_dataErrorModelEnabled)&&(m_rxPacketInfo.size ()>0)&&(itSinr != expectedTbToSinrIndex.end())) // avoid to check for errors when there is no actual data transmitted
            {
              ...
              if (!m_nistErrorModelEnabled)
                {
            	  //NS_LOG_INFO("m_nistErrorModelEnabled = " << m_nistErrorModelEnabled );
                  NistTbStats_t tbStats = NistLteMiErrorModel::GetTbDecodificationStats (m_slSinrPerceived[(*itSinr).second] /*[(*itSinr).second]*4: Average gain for SIMO based on [CatreuxMIMO] */, (*itTb).second.rbBitmap, (*itTb).second.size, (*itTb).second.mcs, harqInfoList);
                  (*itTb).second.mi = tbStats.mi;
                    if(m_slBlerEnabled)
                      {
                        if(!rbDecoded)
                ...
                {
                  NistTbErrorStats_t tbStats = NistLtePhyErrorModel::GetPsschBler (m_fadingModel,NistLtePhyErrorModel::SISO, (*itTb).second.mcs, GetMeanSinr (m_slSinrPerceived[(*itSinr).second] /* [(*itSinr).second]*4: Average gain for SIMO based on [CatreuxMIMO] */, (*itTb).second.rbBitmap),  harqInfoList);
                  (*itTb).second.sinr = tbStats.sinr;
                  NS_LOG_INFO("PSSCH: sinr = "<< tbStats.sinr << " bler = " << tbStats.tbler);
                  if(m_slBlerEnabled)
                    {
                      if (!rbDecoded)
                        {
                          (*itTb).second.corrupt = m_random->GetValue () > tbStats.tbler ? false : true;
                          //std::cout<<"PSSCH "<<(float)GetMeanSinr(m_slSinrPerceived[(*itSinr).second],(*itTb).second.rbBitmap)<<" " << (float)m_mobility-> GetDistanceFrom (paramss->txPhy->GetMobility())<<" "<< (float)tbStats.tbler<<" "<<(float)(*itTb).second.corrupt<<"\n";
                          NS_LOG_INFO("PSSCH: corrupt = "<< (*itTb).second.corrupt);
                        }
                      else
                      {
                    	  NS_LOG_INFO("already decoded");
                      }
                    }
                  NS_LOG_DEBUG (this << " from RNTI " << (*itTb).first.m_rnti << " size " << (*itTb).second.size << " mcs " << (uint32_t)(*itTb).second.mcs << " bitmap " << (*itTb).second.rbBitmap.size () << " TBLER " << tbStats.tbler << " corrupted " << (*itTb).second.corrupt);
              ...
              for (int i=0; i < (int)m_allsendIDs.size();i++)
               {
                if ( m_allsendIDs[i]->nodeId == (*itTb).first.m_rnti)
                {
                	pssch_dist = m_mobility-> GetDistanceFrom (m_allsendIDs[i]->txPhy->GetMobility());
                }
                //NS_LOG_INFO(m_allsendIDs[i]->nodeId);
                }
              params.m_distance = pssch_dist;
              params.m_sinrPerRb = GetMeanSinr (m_slSinrPerceived[(*itSinr).second] /* *itSinr).second]*4: Average gain for SIMO based on [CatreuxMIMO] */, (*itTb).second.rbBitmap);
              ...
              } else
              {
            	  for (std::multiset<NistSlCtrlPacketInfo_t>::iterator itSingalSignal =sortedControlMessagesSingalSignal.begin(); itSingalSignal != sortedControlMessagesSingalSignal.end() ; itSingalSignal++ )
            	   {
            	         if ( (int)(*itSinr).second == (int)(*itSingalSignal).index)
            	          {
            	                 double sinrSingalSignalPSSCH =  (*itSingalSignal).sinr;
            	                 double errorRatePSSCHbis = NistLtePhyErrorModel::GetPsschBler (m_fadingModel,NistLtePhyErrorModel::SISO,(*itTb).second.mcs, sinrSingalSignalPSSCH, harqInfoList).tbler;

            	                 bool ctrlErrorPSSCHbis = m_random->GetValue () > errorRatePSSCHbis ? false : true;
            	                 NS_LOG_INFO("sinrSingalSignal= "<<sinrSingalSignalPSSCH<<" errorRatebis= "<< errorRatePSSCHbis <<" ctrlErrorbis= "<< ctrlErrorPSSCHbis);
            	                 NS_LOG_INFO("sinrSingalSignalPSSCH= "<< sinrSingalSignalPSSCH << " sinrPSSCH= " << GetMeanSinr (m_slSinrPerceived[(*itSinr).second], (*itTb).second.rbBitmap));
            	                 if ( std::abs(sinrSingalSignalPSSCH - GetMeanSinr(m_slSinrPerceived[(*itSinr).second], (*itTb).second.rbBitmap)) < 0.1 )
            	                 {
         	                        reason_PSSCH = "propagation path loss";
            	                 } else
            	                 {
            	                	 if (ctrlErrorPSSCHbis == 0)
            	                	   {
            	                	         reason_PSSCH = "collision";
            	                	   } else
            	                	   {
            	                	          reason_PSSCH = "propagation path loss";
            	                	   }
            	  ...
            	  /*if (reason_PSCCH == "collision" reason_PSCCH == "collision + propagation path loss")
            	  {
                	  reason_PSSCH = reason_PSCCH;
            	  } else {
            		  reason_PSSCH = "propagation path loss";
            	  }*/

              }
              NS_LOG_INFO(reason_PSSCH);
              params.m_reason = reason_PSSCH;
              //params.m_distance = m_mobility->GetPosition().x;
              //params.m_distance = paramss->txPhy->GetMobility()->GetPosition().x;
              //params.m_distance = m_mobility-> GetDistanceFrom (paramss->txPhy->GetMobility());
              params.m_rv = harqInfoList.size ();
      ...
      if ((m_dataErrorModelEnabled)&&(m_rxPacketInfo.size ()>0)&&(itSinr != expectedTbToSinrIndex.end())) // avoid to check for errors when there is no actual data transmitted
        {
          ...
          if (!m_nistErrorModelEnabled)
            {
        	  //NS_LOG_INFO("m_nistErrorModelEnabled = " << m_nistErrorModelEnabled );
              NistTbStats_t tbStats = NistLteMiErrorModel::GetTbDecodificationStats (m_slSinrPerceived[(*itSinr).second]*4, (*itTb).second.rbBitmap, (*itTb).second.size, (*itTb).second.mcs, harqInfoList); //m_slSinrPerceived[(*itSinr).second]*4: Average gain for SIMO based on [CatreuxMIMO]
              (*itTb).second.mi = tbStats.mi;
                if(m_slBlerEnabled)
                  {
                    if(!rbDecoded)
            ...
            {
              NistTbErrorStats_t tbStats = NistLtePhyErrorModel::GetPsschBler (m_fadingModel,NistLtePhyErrorModel::SISO, (*itTb).second.mcs, GetMeanSinr (m_slSinrPerceived[(*itSinr).second]*4, (*itTb).second.rbBitmap),  harqInfoList); //m_slSinrPerceived[(*itSinr).second]*4:Average gain for SIMO based on [CatreuxMIMO]
              (*itTb).second.sinr = tbStats.sinr;
              NS_LOG_INFO("PSSCH: sinr = "<< tbStats.sinr << " bler = " << tbStats.tbler);
              if(m_slBlerEnabled)
                {
          ...
          params.m_ndi = (*itTb).second.ndi;
          params.m_correctness = (uint8_t)!(*itTb).second.corrupt;
          params.m_sinrPerRb = GetMeanSinr (m_slSinrPerceived[(*itSinr).second]*4, (*itTb).second.rbBitmap); //(*itSinr).second]*4: Average gain for SIMO based on [CatreuxMIMO]
          ...
          if (!rbDecoded)
            {
              //if (!m_dropRbOnCollisionEnabled)//uncomment IF_CONDITION if PSCCH BLER is not considered in this mode!
                //{
                  ...
                  if ( m_rxPacketInfo[i].m_rxControlMessage->GetMessageType() == NistLteControlMessage::SCI)
                    {
                      errorRate = NistLtePhyErrorModel::GetPscchBler (m_fadingModel,NistLtePhyErrorModel::SISO, GetMeanSinr (m_slSinrPerceived[i]*4, m_rxPacketInfo[i].rbBitmap)).tbler; //(m_slSinrPerceived[i]*4) Average gain for SIMO based on [CatreuxMIMO]
                      ctrlError = m_random->GetValue () > errorRate ? false : true;
                      NS_LOG_DEBUG (this << " PSCCH Decoding, errorRate " << errorRate << " error " << ctrlError);
                    }
                  else if (m_rxPacketInfo[i].m_rxControlMessage->GetMessageType() == NistLteControlMessage::MIB_SL)
                    {
                      errorRate = NistLtePhyErrorModel::GetPsbchBler (m_fadingModel,NistLtePhyErrorModel::SISO, GetMeanSinr (m_slSinrPerceived[i]*4, m_rxPacketInfo[i].rbBitmap)).tbler; //m_slSinrPerceived[i]*4: Average gain for SIMO based on [CatreuxMIMO]
                      ctrlError = m_random->GetValue () > errorRate ? false : true;
                      NS_LOG_DEBUG (this << " PSBCH Decoding, errorRate " << errorRate << " error " << ctrlError);
                    }
                  else
          ...
          if ((m_ctrlErrorModelEnabled)&&(m_rxPacketInfo.size ()>0)&&(itSinrDisc != expectedTbToSinrDiscIndex.end())) // avoid to check for errors when there is no actual discovery transmitted
            {   
              ...
              if (!m_nistErrorModelEnabled)
                ...
                {
                  NistTbErrorStats_t tbStats = NistLtePhyErrorModel::GetPsdchBler (m_fadingModel,NistLtePhyErrorModel::SISO, GetMeanSinr (m_slSinrPerceived[(*itSinrDisc).second]*4 , (*itTbDisc).second.rbBitmap),  harqInfoList); //m_slSinrPerceived[(*itSinrDisc).second]*4  Average gain for SIMO based on [CatreuxMIMO]
              ...
              params.m_sinrPerRb = GetMeanSinr (m_slSinrPerceived[(*itSinrDisc).second]*4, (*itTbDisc).second.rbBitmap); //(*itSinrDisc).second]*4: Average gain for SIMO based on [CatreuxMIMO]
              params.m_rv = harqInfoList.size ();
              m_slPhyReception (params);  
            }
          itTbDisc++;
              ...
              if (!rbDecoded)
                {
                  ...
                  if (ok)
                    {
                      double  errorRate = NistLtePhyErrorModel::GetPscchBler (m_fadingModel,NistLtePhyErrorModel::SISO, GetMeanSinr (m_slSinrPerceived[i]*4, m_rxPacketInfo[i].rbBitmap)).tbler; //m_slSinrPerceived[i]*4 : Average gain for SIMO based on [CatreuxMIMO]
                      ctrlError = m_random->GetValue () > errorRate ? false : true;
...
NistLteSpectrumPhy::GetMeanSinr (const SpectrumValue& sinr, const std::vector<int>& map)
{
  ...
  //std::cout<<"sinrLin= "<<(float)sinrLin <<" map.size()= "<<(float)map.size()<<" sinr= "<<(float)(sinrLin / map.size())<<"\n";
  return sinrLin / map.size();
}

double
NistLteSpectrumPhy::GetMeanSinrfromSignal (const SpectrumValue& signal, const std::vector<int>& map)
{
  SpectrumValue signalCopy = signal;
  double sinrLin = 0;
  for (uint32_t i = 0; i < map.size (); i++)
    {
	  double sinr = (signalCopy[map.at (i)])/(3.16228e-20);
	  NS_LOG_INFO(signalCopy[map.at (i)]);
...
//added by Adel
Ptr<SpectrumValue>
NistLteSpectrumPhy::GetAllSignals()
{
	  NS_LOG_FUNCTION (this);
	  Ptr<SpectrumValue> allSignals = m_interferenceSl->GetAllSignals();
	  /*if (allSignals)
	  {
		  NS_LOG_INFO ("all signals = "<<(*allSignals));
	  }*/
	  return allSignals;
	  //return m_interferenceSl->GetAllSignals();
