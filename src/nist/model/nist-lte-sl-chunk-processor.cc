...
NS_LOG_COMPONENT_DEFINE ("NistLteSlChunkProcessor");

NistLteSlChunkProcessor::NistLteSlChunkProcessor ()
{
...
void
NistLteSlChunkProcessor::AddCallback (NistLteSlChunkProcessorCallback c)
{
  NS_LOG_FUNCTION (this);
...
void
NistLteSlChunkProcessor::Start (bool init)
{
  NS_LOG_FUNCTION (this);
...
void
NistLteSlChunkProcessor::EvaluateChunk (uint32_t index, const SpectrumValue& sinr, Time duration)
{
  ...
  if (m_chunkValues[0].m_totDuration.GetSeconds () > 0)
    {
      ...
      std::vector<NistLteSlChunkValue>::iterator itValues;
      ...
      {
          NS_LOG_INFO (this << values[i]);
      }*/

      std::vector<NistLteSlChunkProcessorCallback>::iterator it;
