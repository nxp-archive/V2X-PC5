...
MultiModelSpectrumChannel::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::MultiModelSpectrumChannel")
    ...
    .AddAttribute ("MaxLossDb",
                   ...
                   DoubleValue (1.0e9), /*1.0e9*/
                   MakeDoubleAccessor (&MultiModelSpectrumChannel::m_maxLossDb),
                   MakeDoubleChecker<double> ())
    //NIST
    .AddAttribute ("NistPropagationLoss",
                   "a pointer to the m_propagationLoss, added to trace the pathloss value",
                   PointerValue (0),
                   MakePointerAccessor (&MultiModelSpectrumChannel::m_propagationLoss),
                   MakePointerChecker<PropagationLossModel> ())
    .AddTraceSource ("Gain",
                    "We get the transmitter id, receiver id, txAntennaGain, rxAntennaGain, rxPower, pathloss, shadowing, plos, r",
                    MakeTraceSourceAccessor (&MultiModelSpectrumChannel::m_gainTrace),
          ...
          if ((*rxPhyIterator) != txParams->txPhy)
            {
              ...
              NS_LOG_INFO (" initial rxpsd = "<< *(rxParams->psd));
              ...
              if (txMobility && receiverMobility)
                {
                  ...
                  NS_LOG_LOGIC ("m_maxLossDb = " << m_maxLossDb << " dB");
                  if ( pathLossDb > m_maxLossDb)
                    {
                      // beyond range
                	  NS_LOG_LOGIC ("beyond range ");
                      continue;
                    }
                  double pathGainLinear = std::pow (10.0, (-pathLossDb) / 10.0);
                  *(rxParams->psd) *= pathGainLinear;              
            	  NS_LOG_INFO (" rxpsd = "<< *(rxParams->psd)); //not true in this case
            	  NS_LOG_INFO (" pathGainLinear = "<< pathGainLinear);
                  if (m_spectrumPropagationLoss)
                    {
                	  //NS_LOG_LOGIC (" m_spectrumPropagationLoss is true "); //not true in this case
              ...
              //NS_LOG_LOGIC ("propagation Delay = " << delay <<" m propagation Delay = " << m_propagationDelay);
